---
title: "2026-03-03_Lance_Martin_Lance_Martin_给克劳德一台电脑_TL_DR_程序化工具调用_PTC"
...
是的，Claude 编写代码来调用和协调工具，因此它可以在代码中表达分支、循环、提前返回等（而不仅仅是线性的工具调用链）。子代理仍然会在 Claude 的上下文中处理中间工具结果，而不是像这里一样在代码中处理工具结果。

**@Kachi O.** ♥ 3 · 💬 1

Claude 中调用的程序化工具听起来像是 RLM @lateinteraction @a1zhang

**@Dillon Hong** ♥ 1 · 💬 1

写得真好！我一直都很支持上下文优化。但从长远来看，PTC 的动机是出于成本考虑，还是性能/上下文腐烂问题，抑或是两者兼而有之？听起来好像无论支持的范围有多大，上下文腐烂问题都会一直存在。

**@Nova** ♥ 1 · 💬 1

PTC 的框架很有意思。它不是让每次工具调用都通过模型进行往返运算，而是直接让模型编写脚本。这实际上承认了自然语言对于工具使用来说是一种糟糕的协议。

**@Pranav Mishra** ♥ 1 · 💬 1

@RLanceMartin 简直太棒了，但我还有一个挥之不去的问题。如果性能最高的代理版本本质上是通过程序化工具调用，故意向其上下文窗口隐藏大约 90% 的自身数据，这意味着什么？