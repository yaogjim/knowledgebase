---
title: "2025-11-19_research_google_httpsstorage_googleapis_comgweb_research20"
source: "https://research.google/blog/generative-ui-a-rich-custom-visual-interactive-user-experience-for-any-prompt/"
author:
  - "[[@research.google]]"
published: 2025-11-19
created: 2025-11-19
description:
tags:
  - "research"
  - "@research.google"
  - "ai"
  - "https"
---

# ![](httpsstorage.googleapis.comgweb-research20

![](https://storage.googleapis.com/gweb-research2023-media/original_images/GenUI-0b-Hero.png)

## 生成式界面：为任意提示词打造丰富、定制化、可视化的交互式用户体验

2025 年 11 月 18 日

Google 院士 Yaniv Leviathan、高级软件工程师 Dani Valevski，以及 Google 研究副总裁兼负责人 Yossi Matias

我们推出了一种创新的生成式用户界面实现方式，使人工智能模型能够即时为任何指令创建沉浸式体验、交互工具及动态模拟。这项技术现已在 Gemini 应用和 Google 搜索中率先通过 AI 模式推出。

Labels:- [Generative AI](/blog/label/generative-ai)
- [人机交互与可视化](/blog/label/human-computer-interaction-and-visualization)
- [Product](/blog/label/product)

### 其他相关文章

- [
 
 ![](https://storage.googleapis.com/gweb-research2023-media/original_images/NestedLearning-0-Hero.png)
 
 November 7, 2025
 
 推出嵌套学习：持续学习的新机器学习范式
 
 - Algorithms & Theory ·
 - Generative AI ·
 - 机器智能
 
 
 
 ](/blog/introducing-nested-learning-a-new-ml-paradigm-for-continual-learning/)
- [
 
 ![](https://storage.googleapis.com/gweb-research2023-media/original_images/ResearchAt25_Hero.jpg)
 
 October 31, 2025
 
 加速研究突破与现实应用的良性循环
 
 - 气候与可持续性 ·
 - Generative AI ·
 - Health & Bioscience ·
 - Quantum
 
 
 
 ](/blog/accelerating-the-magic-cycle-of-research-breakthroughs-and-real-world-applications/)
- [
 
 ![](https://storage.googleapis.com/gweb-research2023-media/original_images/ConfFedAnalytics1_Hero.png)
 
 October 30, 2025
 
 迈向可证明的 AI 使用隐私洞察
 
 - Generative AI ·
 - Mobile Systems ·
 - 安全、隐私与滥用防范 ·
 - 软件系统与工程
 
 
 
 ](/blog/toward-provably-private-insights-into-ai-use/)